...
namespace wcommsixwrap
{
    class WinCatalog
    {
        ...
        private int catalogVersion1 = 256;

        // Catalog Version is (0X200 = 512) for Catalog Version 2
        private int catalogVersion2 = 512;
        private string catalogFilePath;
        public int catalogVersion;

        public WinCatalog(string catalogFilePath)
        {
        ...
        public HashSet<String> GetHashesFromCatalog()
        {
        ...
        private int GetCatalogVersion(IntPtr catalogHandle)
        {
        ...
        public string CalculateFileHash(string filePath, string hashAlgorithm)
        {
        ...
        public X509Certificate2 GetAuthenticodeSignature()
        {
            ...
            try
            {
                ...
                var fileContent = File.ReadAllBytes(catalogFilePath);
            ...
            {
                Console.WriteLine($"Error retrieving Authenticode signature: {ex.Message}");
            }

            return signerCert;
        }

        public bool IsFileInCatalog(string filePath)
        {
            HashSet<String> fileHashes = this.GetHashesFromCatalog();
            var fileHash = CalculateFileHash(filePath, "SHA256");
            foreach (var member in fileHashes)
            {
                if (member == fileHash)
                    return true;
            }
